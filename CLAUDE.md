...
5. Optional: import into Shopify test stores via `/api/customer-import/*`. The import sends the **final template dataset** (`reports/templateDataset.ts`: column mapping + merge-matching-duplicates + move-invalid-contact-to-Notes + move-duplicates-to-Notes + fill-missing-contact-name, same transformation as the Excel "Shopify Template" sheet — not the raw rows).
   ...
   **Cleanup-option order inside `buildTemplateDataset` is load-bearing.** Invalid values are stripped *before* duplicate grouping (a cleared email cannot also be reported as a duplicate), and the placeholder name is filled *last*, after every transform that can empty a row. `fillMissingContactName` consults a `hadSubstance` snapshot taken before any transform runs — read later, a blank CSV line looks substantial because the strip just wrote a Note into it. Blank rows are never named: mass `MissingContact` is how an operator discovers they mapped the wrong columns, and that alarm has to keep ringing. The reconcile rebuilds this dataset deterministically to map bulk-result lines back to CSV rows, so the transformation must stay a pure function of (originalRows, mapping, flags).
