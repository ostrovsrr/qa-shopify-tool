...
// runs surface the accepted/rejected split so the outcome is obvious at a glance.
function ImportBadge({ lastImport }: { lastImport: ProductHistoryImport | null }) {
...
export function ProductHistory({ onOpen, refreshTrigger }: Props) {
              ...
              <div className="history-item-info">
                ...
                {(item.ticketNumber || item.ticketName || item.comments) && (
                  <div className="history-metadata-summary">
                    {item.ticketNumber && (
                      <span className="metadata-chip metadata-chip-ticket">#{item.ticketNumber}</span>
                    )}
                    {item.ticketName && (
                      <span className="metadata-chip metadata-chip-name">{item.ticketName}</span>
                    )}
                    {item.comments && (
                      <span className="metadata-chip metadata-chip-comments" title={item.comments}>
                        💬 {item.comments.length > 60 ? item.comments.slice(0, 60) + '…' : item.comments}
                      </span>
                    )}
                  </div>
                )}
